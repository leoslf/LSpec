...
-- where
--  options' := addHelpFlag $ options.map $ Functor.map interpretOptions
...
--
-- def interpretResult {Config} (config : Config) : List (Config -> Except InvalidArgument Config) × List String × List String -> Except String Config := sorry
-- --  interpretGetOptResult >=> foldResult config
--
